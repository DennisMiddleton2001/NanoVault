...
class IngestPipeline:
    ...
    def __init__(self, env = None):
        ...
        if argc == 4 and self.command in ['--si', '--st']:
            self.model_path = sys.argv[2]
            self.model_name = sys.argv[3]
        else:
            self.workflow_path = sys.argv[2]

    def run(self, workflow_path=None):
        if self.command in ['--r']:
            self.vault.refresh_vault()
        elif self.command in ['--si']:
            self.single_ingest(delete_source=False)
        elif self.command in ['--st']:
            self.single_ingest(delete_source=True)
        else:
            self.run_from_workflow()
    
    def run_from_workflow(self, delete_source=False):
        
        parser = WorkflowParser(self.env, self.workflow_path)
        ...
        print(f"{self.env.ico.get("ACT",__class__)} Starting template-driven INGEST for: {os.path.basename(self.workflow_path)}")
        print(f"{self.env.ico.get("SRCH",__class__)} Found {len(required_models)} model dependencies to evaluate.\n")
        ...
        for model in required_models:
            ...
            if vault_path:
                ...
                if not self.vault.read_sidecar(vault_path):
                    ...
                    active_hash = self.vault.calculate_hash(active_path)
            ...
            if not manifest["valid"]:
                print(f"{self.env.ico.get('ERR',__class__)} Active file is structurally invalid! Ingestion aborted.")
                print(f"       -> Details: {manifest['error']}")
                print(self.env.ico.sep(2))
                continue
            else:
                print(f"{self.env.ico.get('DONE',__class__)} File structure validated.")
            ...
            # If it this instantly copies it BACK to the NVMe so you can still use it
            if not manifest["valid"]:
                self.vault.deploy_to_active(new_vault_path, subfolder, name)
            print(self.env.ico.sep(2))
        print(self.env.ico.sep(1))
